...
VL53L1X_ERROR VL53L1X_SensorInit(uint16_t dev) {
  ...
  status |= VL53L1_WrByte(dev, VL53L1_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND,
                          0x09); /* two bounds VHV */
  status |=
...
VL53L1X_ERROR VL53L1X_SetInterruptPolarity(uint16_t dev, uint8_t NewPolarity) {
  ...
  Temp = Temp & 0xEF;
  status |=
      VL53L1_WrByte(dev, GPIO_HV_MUX__CTRL, Temp | (!(NewPolarity & 1)) << 4);
  return status;
}
...
VL53L1X_ERROR VL53L1X_GetDistance(uint16_t dev, uint16_t *distance) {
  ...
  status |= (VL53L1_RdWord(
      dev, VL53L1_RESULT__FINAL_CROSSTALK_CORRECTED_RANGE_MM_SD0, &tmp));
  *distance = tmp;
  return status;
}
...
VL53L1X_ERROR VL53L1X_GetSignalPerSpad(uint16_t dev, uint16_t *signalRate) {
  ...
  status |= VL53L1_RdWord(
      dev, VL53L1_RESULT__PEAK_SIGNAL_COUNT_RATE_CROSSTALK_CORRECTED_MCPS_SD0,
      &signal);
  status |=
      VL53L1_RdWord(dev, VL53L1_RESULT__DSS_ACTUAL_EFFECTIVE_SPADS_SD0, &SpNb);
...
VL53L1X_ERROR VL53L1X_GetSpadNb(uint16_t dev, uint16_t *spNb) {
  ...
  status |=
      VL53L1_RdWord(dev, VL53L1_RESULT__DSS_ACTUAL_EFFECTIVE_SPADS_SD0, &tmp);
  *spNb = tmp >> 8;
  return status;
}
...
VL53L1X_ERROR VL53L1X_SetOffset(uint16_t dev, int16_t OffsetValue) {
  ...
  Temp = (OffsetValue * 4);
  status |=
      VL53L1_WrWord(dev, ALGO__PART_TO_PART_RANGE_OFFSET_MM, (uint16_t)Temp);
...
VL53L1X_ERROR VL53L1X_SetXtalk(uint16_t dev, uint16_t XtalkValue) {
  ...
  status |= VL53L1_WrWord(
      dev, ALGO__CROSSTALK_COMPENSATION_X_PLANE_GRADIENT_KCPS, 0x0000);
  status |= VL53L1_WrWord(
      dev, ALGO__CROSSTALK_COMPENSATION_Y_PLANE_GRADIENT_KCPS, 0x0000);
  status |= VL53L1_WrWord(
...
VL53L1X_ERROR VL53L1X_GetXtalk(uint16_t dev, uint16_t *xtalk) {
  ...
  status |=
      VL53L1_RdWord(dev, ALGO__CROSSTALK_COMPENSATION_PLANE_OFFSET_KCPS, xtalk);
  *xtalk =
      (uint16_t)((*xtalk * 1000) >>
...
VL53L1X_ERROR VL53L1X_SetROI(uint16_t dev, uint16_t X, uint16_t Y) {
  ...
  status |= VL53L1_RdByte(dev, VL53L1_ROI_CONFIG__MODE_ROI_CENTRE_SPAD,
                          &OpticalCenter);
  if (X > 16)
    X = 16;
  if (Y > 16)
...
VL53L1X_ERROR VL53L1X_GetROI_XY(uint16_t dev, uint16_t *ROI_X,
  ...
  status =
      VL53L1_RdByte(dev, ROI_CONFIG__USER_ROI_REQUESTED_GLOBAL_XY_SIZE, &tmp);
...
VL53L1X_ERROR VL53L1X_SetSignalThreshold(uint16_t dev, uint16_t Signal) {
  ...
  status |= VL53L1_WrWord(dev, RANGE_CONFIG__MIN_COUNT_RATE_RTN_LIMIT_MCPS,
                          Signal >> 3);
  return status;
}
...
VL53L1X_ERROR VL53L1X_GetSignalThreshold(uint16_t dev, uint16_t *signal) {
  ...
  status |=
      VL53L1_RdWord(dev, RANGE_CONFIG__MIN_COUNT_RATE_RTN_LIMIT_MCPS, &tmp);
  *signal = tmp << 3;
  return status;
}
